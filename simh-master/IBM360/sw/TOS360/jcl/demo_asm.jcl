// JOB DEMOASM ECLIPTIC COORDINATE TABLE
// OPTION LINK
// EXEC ASSEMBLY
ECLP     TITLE 'ECLIPTIC COORDINATE TABLE'
* SAMPLE TOS ASSEMBLER LANGUAGE PROGRAM
* DEMONSTRATES TRAPPING PROGRAM INTERRUPTIONS
* DEMONSTRATES DOUBLE PRECISION FLOATING POINT INSTRUCTIONS
* DEMONSTRATES CALLING A FORTRAN FUNCTION
* USES PRINTER ASA CHARACTERS
*        PRINT NOGEN
ECLIPTIC START X'2000'
R0       EQU   0
R1       EQU   1
R2       EQU   2
R3       EQU   3
...
FR6      EQU   6
         BALR  R12,0
         USING *,R12
         ST    R12,SAVEBASE            SAVE BASE REGISTER
         STXIT PC,THATSALL,SAVEXIT
         STXIT IT,THATSALL,SAVEXIT
         SETIME 15
         LD    FR0,TWOPI               PI 
         MD    FR0,=D'2'                TIMES 2
...
LOOP1    EQU   *
         ...
         AD    FR4,=D'1'
         STD   FR4,MIN
         BCT   R9,LOOP1               LOOP FOR NEXT PART OF HOUR
...
*
         CLOSE PRINTER
         EOJ
THATSALL BALR  R11,0                  IF PROGRAM CHECK ABEND,
         USING *,R11
         L     R12,SAVEBASE
         DROP  R11
...
SAVEXIT  DS    9D                     PROGRAM CHECK SAVE AREA
DOUBLE   DS    D                      WORK AREA
SAVEBASE DS    F                      BASE REGISTER
...
PRNTOUT1 DS    CL133
         ...
         END
/*
 INCLUDE IJTLSCN   BASIC FORTRAN IV SINE/COSINE FUNCTION
// EXEC LNKEDT
// EXEC
/&
